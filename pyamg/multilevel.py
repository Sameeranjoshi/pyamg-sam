...
class MultilevelSolver:
    ...
    def solve_3_phase_cycle(self, b, x0=None, tol=1e-5, maxiter=100, cycle='V', accel=None,
              callback=None, residuals=None, cycles_per_level=1, return_info=False):
        """Execute multigrid cycling.
        ...
        while True:  # it <= maxiter and normr >= tol:
            if len(self.levels) == 1:
                ...
                self._solve_3_phase_cycle(0, x, b, cycle, cycles_per_level)
    ...
    def _solve_3_phase_cycle(self, lvl, x, b, cycle, cycles_per_level=1):
        """Multigrid cycling split into 3 parts."""
        stack = []
        # Go down to the coarse grid
        self._solve_down(stack, lvl, x, b, cycle, cycles_per_level)
        # access the last element from the list
        coarse_x, coarse_b = stack[-1][-1], stack[-1][-2]
        lvl = stack[-1][0]
        # # Solve at the lowest grid level
        coarse_x = self._solve_lowest(lvl, coarse_x, coarse_b)
        # # Apply the coarse grid correction and postsmooth
        self._solve_up(stack, lvl, coarse_x, coarse_b)

    def _solve_down(self, stack, lvl, x, b, cycle, cycles_per_level=1):
        """Handles the recursive step to go down to a coarser grid."""
        A = self.levels[lvl].A
        # Apply the presmoother
        self.levels[lvl].presmoother(A, x, b)
        ...
        coarse_b = self.levels[lvl].R @ residual
        coarse_x = np.zeros_like(coarse_b)
        stack.append((lvl, residual, x, b, coarse_b, coarse_x))
        # Recurse down to the coarser grid
        if lvl < len(self.levels) - 2:  # Not the coarsest grid
            if cycle == 'V':
                self._solve_down(stack, lvl + 1, coarse_x, coarse_b, 'V')
            elif cycle == 'W':
                self._solve_down(stack, lvl + 1, coarse_x, coarse_b, cycle)
                self._solve_down(stack, lvl + 1, coarse_x, coarse_b, cycle)
            elif cycle == 'F':
                self._solve_down(stack, lvl + 1, coarse_x, coarse_b, cycle, cycles_per_level)
                for _ in range(0, cycles_per_level):
                    self._solve_down(stack, lvl + 1, coarse_x, coarse_b, 'V', 1)
            elif cycle == 'AMLI':
                self._solve_amli(lvl, coarse_x, coarse_b, cycle)
            else:
                raise TypeError(f'Unrecognized cycle type ({cycle})')

    def _solve_lowest(self, lvl, coarse_x, coarse_b):
        """Handles the solve at the lowest level (before it becomes coarse)."""
        if lvl == len(self.levels) - 2:
            coarse_x[:] = self.coarse_solver(self.levels[-1].A, coarse_b)
        return coarse_x
    
    def _solve_up(self, stack, lvl, coarse_x, coarse_b):
        """Handles the coarse grid correction and postsmoothing."""
        while stack:
            lvl, _, x, b, coarse_b, coarse_x = stack.pop()  # Get the stored values
            # print("solve up", lvl)
            # print(f"x shape: {x.shape}")
            # print(f"coarse_x shape: {coarse_x.shape}")
            # print(f"P shape: {self.levels[lvl].P.shape}")

            # Apply the coarse grid correction
            x += self.levels[lvl].P @ coarse_x  

            # Apply the postsmoother
            A = self.levels[lvl].A
            self.levels[lvl].postsmoother(A, x, b)
